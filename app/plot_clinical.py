...
def plot_clinical(projects):
    ...
    if (ICD_O_3_SITE_CODE in clinical_vars) and (ICD_O_3_SITE_DESC in clinical_vars):
        clinical_df[ICD_O_3_SITE_CODE] = clinical_df.apply(
            lambda row: append_icd_desc(row, ICD_O_3_SITE_CODE, ICD_O_3_SITE_DESC), 
            axis='columns'
        )
    if (ICD_O_3_HISTOLOGY_CODE in clinical_vars) and (ICD_O_3_HISTOLOGY_DESC in clinical_vars):
